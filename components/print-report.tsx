...
export function PrintReport({ site }: { site: Site }) {
  const { t, tType, tStatus, tCondition, tBreakdown, tInspectionCat, tInspectionNote } =
    useI18n();

  const version = `${site.reference}-R${site.documents.length}.0`;
  ...
  return (
    <div className="min-h-screen bg-muted/50 print:bg-white">
      <PrintToolbar backHref={`/sites/${site.id}`} />

      <div className="mx-auto max-w-4xl px-4 py-8 print:p-0">
        <div className="print-page mx-auto rounded-lg border bg-white p-5 text-[13px] leading-relaxed text-slate-800 shadow-lg sm:p-10 print:rounded-none print:border-0 print:p-0 print:shadow-none">
          {/* Confidential ribbon */}
          <div className="mb-4 flex items-center justify-between gap-2 rounded-md bg-slate-900 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-widest text-white print:rounded-none">
            <span>{t.print.confidential}</span>
            <span className="text-white/60">{t.print.valuationReport}</span>
          </div>
          ...
          {/* Headline valuation */}
          <div className="grid grid-cols-1 gap-3 rounded-lg bg-slate-50 p-4 sm:grid-cols-3 sm:gap-4 sm:p-5 print:rounded-none">
            <div>
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-slate-900">{site.confidence}%</p>
            </div>
          </div>
              ...
              <tbody>
                {site.valuationBreakdown.map((line) => (
                  <tr key={line.key} className="border-b border-slate-100">
                    <td className="py-2 text-slate-700">{tBreakdown[line.key]}</td>
              ...
              <tbody>
                {site.inspection.map((item) => (
                  <tr key={item.key} className="border-b border-slate-100 align-top">
                    ...
                    <td className="py-2 text-slate-600">{tInspectionNote[item.key]}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="mt-8 grid grid-cols-2 gap-8 border-t border-slate-300 pt-6">
            <div>
              ...
              <p className="text-xs text-slate-500">{site.surveyor.role}</p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-wide text-slate-400">
          ...
          {/* On-page stamp */}
          <PrintStamp printedBy={currentUser.name} version={version} />

          <p className="mt-4 text-[10px] leading-relaxed text-slate-400">
            {t.print.confidentialNote}
          </p>
          <p className="mt-2 border-t border-slate-200 pt-3 text-[10px] leading-relaxed text-slate-400">
            {t.print.disclaimer} © {new Date().getFullYear()} {currentUser.org}.
          </p>

          {/* Repeating confidential footer (fixed when printing) */}
          <PrintFooter printedBy={currentUser.name} version={version} />
        </div>
      </div>
    </div>
  );
